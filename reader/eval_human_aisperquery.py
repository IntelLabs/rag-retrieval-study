...
def compute_autoais(data,
    ...
    for item_idx, item in enumerate(tqdm(data)):
        ...
        if qampari:  # split by comma instead of tokenizing (strips off whitespace, periods and commas before split)
            ...
            sents = [s['text'] for s in item['sentences']]
        else:
        ...
        if noise:
            ...
            # Not applicable for human eval -- no prompt doc, and already trimmed ndocs
...
def main(args):
    ...
    for d in data:
        d['generated_output'] = d['output']
        d['output'] = d['gold_docs']
    # Data to compare from human eval
    data_for_results = {}
    query_keys_to_keep = ['overall_precision_score', 'overall_recall_score', 'automatic_recall_scores', 'automatic_citation_precision_scores', 'automatic_precision_scores']
    for qk in query_keys_to_keep:
        data_for_results[qk] = [d[qk] for d in data]
    sent_keys = ['sentence_recall_score', 'sentence_precision_score']
    cite_key = 'citation_precision_score'
    human_ratings = []
    for d in data:
        sent_ratings = []
        for sdict in d['sentences']:
            out_dict = {}
            for sk in sent_keys:
                out_dict[sk] = sdict[sk]
            out_dict[cite_key] = [sdc[cite_key] for sdc in sdict['citations']]
            sent_ratings.append(out_dict)
        assert len(sent_ratings) == len(d['sentences']), "Unexpected num of sents from human ratings"
        human_ratings.append(sent_ratings)
    assert len(human_ratings) == len(data), "Unexpected number of human rating items"
    data_for_results['human_ratings'] = human_ratings 
    ...
    result = {}
    result.update(data_for_results)
    if "asqa" in args.f:
