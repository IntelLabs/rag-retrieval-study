...
class CollectionIndexer():
    ...
    def _build_ivf(self):
        ...
        codes = np.zeros(self.num_embeddings, dtype=np.int64)
        if self.verbose > 1:
            print_memory_stats(f'RANK:{self.rank}')
        ...
        assert offset+chunk_codes.size(0) == codes.shape[0], (offset, chunk_codes.size(0), codes.shape)
        if self.verbose > 1:
        ...
        ivf_lengths = np.bincount(codes, minlength=self.num_partitions)
        #ivf_lengths = torch.bincount(values, minlength=self.num_partitions)

        if self.verbose > 1:
            print('Converting numpy to torch')
        ivf = torch.from_numpy(ivf)
        ivf_lengths = torch.from_numpy(ivf_lengths)
